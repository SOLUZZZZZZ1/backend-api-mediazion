...
SESSION_TTL_SECONDS = int(os.getenv("SESSION_TTL", "2592000"))  # 30 days
...
def get_current_user(authorization: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    ...
    conn = db()
    row = conn.execute("SELECT s.expires_at, u.id, u.email, u.status FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token=?", (token,)).fetchone()
...
def mediador_register(data: MediadorRegisterIn):
    ...
    try:
        conn.execute(
            ...
            "VALUES (?,?,?,'pending',?,?,?,?,?,?)",
            (name, email, pwd_hash, created, data.telefono or "", data.bio or "", data.provincia or "", data.especialidad or "", data.web or "", data.linkedin or "")
        )
        conn.execute("INSERT OR IGNORE INTO users (email,password_hash,status,created_at) VALUES (?,?, 'pending', ?)",
...
def get_profile(user=Depends(get_current_user)):
    conn = db()
    m = conn.execute("SELECT name,telefono,bio,provincia,especialidad,web,linkedin,photo_url,cv_url,is_subscriber,status FROM mediadores WHERE email=?", (user["email"],)).fetchone()
    conn.close()
    return {"user": user, "mediador": (dict(m) if m else {})}
...
async def webhook(req: Request):
    ...
    if event.get("type") == "checkout.session.completed":
        ...
        if email:
            conn = db()
            conn.execute("UPDATE mediadores SET is_subscriber=1 WHERE email=?", (email.lower(),))
            # ensure user exists
