...
# Cliente S3 (usará AWS_ACCESS_KEY_ID y AWS_SECRET_ACCESS_KEY del entorno)
...
async def upload_file(file: UploadFile = File(...)):
    ...
    Tu frontend llama a: POST /api/upload/file
    """
    if not file or not file.filename:
        raise HTTPException(status_code=400, detail="No se recibió archivo")

    try:
        # Extensión original, si la tiene
        ...
        # Subir a S3 (sin ACLs, porque tu bucket tiene las ACL desactivadas)
