...
CREATE TABLE IF NOT EXISTS agenda (
  ...
  caso_id         INTEGER,
  created_at      TIMESTAMP DEFAULT NOW()
  -- Si tienes tabla casos y quieres FK:
  -- , FOREIGN KEY (caso_id) REFERENCES casos(id) ON DELETE SET NULL
);
"""

@router.post("/agenda/init")
def agenda_init(x_admin_token: str | None = Header(None)):
    """Crea la tabla AGENDA si no existe."""
    _auth(x_admin_token)
    try:
        with pg_conn() as cx:
            with cx.cursor() as cur:
            ...
            cx.commit()
        return {"status": "ok", "message": "Agenda lista."}
    except Exception as e:
        raise HTTPException(500, f"Agenda init error: {e}")
