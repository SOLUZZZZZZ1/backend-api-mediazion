...
# --- DB init (PostgreSQL)
from utils_pg import ensure_db  # usa DATABASE_URL de Render
...
try:
    from db_routes import db_router  # solo si tienes /db/health
except Exception:
    db_router = None
...
def parse_origins() -> list[str]:
    """Dominios explícitos permitidos desde env (coma-separados)."""
...
# Asegura esquema
ensure_id = ensure_db  # evitar confusiones con 'ensure()'
ensure_id()
...
app.add_middleware(
    ...
    allow_origin_regexp=r"https://.*\.vercel\.app$",
...
# --- Routers ---
app.add_api_route("/health", lambda: {"ok": True, "service": "backend-api-mediazion-1", "version": "3.2.1"}, methods=["GET"])
...
# ⚠️ Temporal: quítalo cuando confirmes que la BD está bien migrada
