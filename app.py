# app.py — MEDIAZION backend (FastAPI) · doble montura para mediadores (/api y legacy sin /api)
import os
...
app = FastAPI(title="MEDIAZION Backend", version="3.2.6")
...
@app.get("/health")
def health():
    return {"ok": True, "service": "backend-api-mediazion-1", "version": "3.2.6"}
...
# Montaje nuevo (con /api)
_safe_include(mediadores_router, prefix=API_PREFIX, tags=["mediadores"])
# Compatibilidad legacy (sin /api) para formularios existentes
_safe_include(mediadores_router, prefix="", tags=["mediadores-legacy"])
...
# ---- Admin manage / DB / Migrate (opcionales)
try:
